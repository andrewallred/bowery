...
function play(out,ix)
  ...
  if rhythm[ix+2][ step[ix+2] ] & 8 == 8 then
    ...
    if out == 1 then 
        ...
        --print(tscale)
        --s = scale[tscale]
        s = chords1[input[2].volts > -0.04 and 1 or 2]
        nn = s[ note%(#s) + 1 ] + transposition1
        oct = math.floor(note/12)
        output[out].volts = nn/12 + oct
    end    
    if out == 3 then
        s = chords2[input[2].volts > -0.04 and 1 or 2]
        nn = s[ note%(#s) + 1 ] + transposition2
  ...
  if totalcount % 255 == 0 then
    local selectedTransposition = math.floor(math.random(#transpositions)) + 1
    print(selectedTransposition)
    transposition1 = transposition1 + transpositions[selectedTransposition]
    print('------------------------------------- 1')
    print(transposition1)
    if transposition1 > 10 then
        transposition1 = 0
    end
  end
  if totalcount % 257 == 0 then
    local selectedTransposition = (math.floor(math.random(#transpositions)) + 1) % #transpositions
    print(selectedTransposition)
    transposition2 = transposition2 + transpositions[selectedTransposition]
    print('------------------------------------- 2')
    print(transposition2)
    if transposition2 > 10 then
        transposition2 = 0
    end
  end
